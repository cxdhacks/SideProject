# https://codingandfun.com/analysing-balance-sheet-financial-ratios-with-python/
import requests
import pandas as pd 

companies = ['AMZN','AAPL','MSFT']
BS_over_time = ''
BS_companies = ''
def balance_sheet(quarter,company,type_analysis):
  api_key = '5be095e960395bbf5fc5c92d933db058'
  ...
  if type_analysis == 'time':
    ...
    BS_metrics[date] = {} # 在 dict 中嵌套一个新的空 dict
...
if len(companies) == 1:
  ...
  BS_over_time = pd.concat([pd.DataFrame(l) for l in Balance_sheet_all],axis=1).T  #.T 表示 transpose
