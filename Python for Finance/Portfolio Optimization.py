...
RF = 0
# Retrieving stock prices and calculating daily returns
empresas = {}
api_key = 'your api key'
#Get all prices into a dataframe
for stock in stocks:
  # 最先 get 的 prices 是 dict
  ...
  prices = prices['historical'][-900:] # 这是 list，由一个个 dict 组成

  prices = pd.DataFrame(prices) 
  
  empresas[stock] = prices.set_index('date') # 这是一个 dataframe，即 key 是 stock 名称，value 是 dataframe
...
for portfolio in range (number_of_portfolios):
  ...
  portfolio_variance = np.dot(weights.T,np.dot(matrix_covariance_portfolio, weights)) # dot product two arrays.
